...
function fillAndSearch(title) {
    ...
    if (input) {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${safeTitle} - 发现TV`;
        } catch (e) {
...
async function fetchDoubanData(url) {
    // 使用 allorigins.win 作为豆瓣 API 数据代理
    // 注意：图片代理使用独立的 /api/proxy 端点
...
function renderDoubanCards(data, container) {
    ...
    if (!data.subjects || data.subjects.length === 0) {
        ...
        data.subjects.forEach(item => {
            ...
            // 2. 也准备代理URL作为备选 - 使用服务器端图片代理解决防盗链问题
            const proxiedCoverUrl = '/api/proxy?url=' + encodeURIComponent(item.cover || '');
